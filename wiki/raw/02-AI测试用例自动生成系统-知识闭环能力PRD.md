...
**核心作用**：依托企业通用知识、项目专属知识、业务流依赖、历史经验、质量规则，保障AI生成用例的覆盖度、规范性、合规性。
...
用户上传PRD、业务代码、JIRA缺陷素材后，系统自动识别业务代码并送入TF\-IDF切片模块，全量重构业务流依赖图谱。本操作为知识体系初始化动作，为后续AI用例生成提供最新的业务流依赖逻辑支撑（TCGF 生成已改用业务流依赖图谱 `business-graph`，不再依赖代码图谱），无正式知识库变更。
