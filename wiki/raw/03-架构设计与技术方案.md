...
### 4.2 知识系统代码图谱更新（后台数据驱动，KS 内部）

代码上传或知识库业务数据变更时，知识系统自动（全量/增量）刷新 B3（代码图谱 `GET /api/graph-data`），供其内部使用；前端（TCGF）通过 `GET /api/business-graph` 只读可视化**业务流依赖图谱**（黑盒测试不依赖源码），**无人工编辑入口**（对齐 PRD 5.3）。
...
| `Context-Harvester` | 生成前只读采集知识上下文 | `/api/brain/pages`, `/api/business-graph`, `/api/search` |
