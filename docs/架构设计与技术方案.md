...
2. `Context-Harvester` 异步从知识系统采集上下文（**只读**）：
   ...
   - 业务流依赖图谱（黑盒测试不依赖源码）：`GET /api/business-graph`（业务侧唯一图谱来源），含业务步骤/API 节点与可测试业务流；知识系统仍保留代码图谱 `GET /api/graph-data`（B3）供其内部使用。
...
sequenceDiagram
    ...
    U->>F: ② 触发生成(大纲/条目/脚本)
    F->>B: 业务意图 op=gen_*
    B->>KS: 采集上下文(只读): brain/pages, business-graph, search
    KS-->>B: 知识上下文
...
> 本节记录 V1.0 中已预置占位、但数据能力需到 V2.0 才具备的功能，供后续迭代对齐。
...
\[
\text{自动化测试覆盖率} = \frac{\text{被自动化脚本覆盖的测试用例数}}{\text{总测试用例数}} \times 100\%
\]

- **分母（总测试用例数）**：取知识系统 `/api/brain/stats` 中 `test-cases` 分类的 `count`，V1.0 已可获取。
- **分子（被自动化脚本覆盖的测试用例数）**：指存在对应自动化测试脚本（`test-scripts` 分类）覆盖的测试用例条目数量。

**V1.0 现状**：分子所需的「脚本 ↔ 用例」覆盖映射在 V1.0 尚未建立，前端在项目空间测试概要中该指标显示「暂无数据」（对应 `showProjectSummary` 中 `coverage` 占位）。

**V2.0 统计方法（待实现）**

1. **覆盖关系建立**：每条自动化脚本在入库（`test-scripts`）时，通过 frontmatter 记录其覆盖的测试用例标识列表（如 `covers: [tc-<uuid>, ...]`），或由 V2 关联分析模块基于标题/步骤语义自动推导脚本与用例的对应关系。
2. **分子统计**：遍历 `test-scripts` 全部条目，汇总去重后的被覆盖测试用例标识数，得到「被自动化脚本覆盖的测试用例数」。
3. **指标落地**：在知识系统新增聚合端点（或在 `/api/brain/stats` 扩展 `automation-coverage` 字段）返回分子与覆盖率；前端 `showProjectSummary` 据此将「暂无数据」替换为真实百分比。
4. **边界处理**：无测试用例时覆盖率为 `0%`；有脚本但无覆盖映射时标记为「未关联」而非误算为 0，避免误导。

### 11.2 用户身份（V2.0）

V1.0 为单用户本地形态，顶部 `本地 · 单用户` 芯片仅作部署形态占位。V2.0 接入认证后，由登录态填充用户名（见 `app.v2.js` 的 `_user` 变量），该芯片即作为「当前用户」指示。
